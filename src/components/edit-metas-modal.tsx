...
export function EditMetasModal({
  ...
  return (
    ...
    >
      <div
        className="my-8 flex max-h-[88vh] w-full max-w-5xl flex-col overflow-hidden rounded-2xl border border-[#e5e7eb] bg-[#f3f4f6] shadow-2xl"
      ...
      >
        ...
        <div className="flex-1 overflow-auto px-6 py-5">
                  ...
                  g.cols.map((c, ci) => (
                    <th
                      ...
                      className={`pb-2 px-2 text-right font-medium ${
                        gi > 0 && ci === 0 ? "border-l border-[#e5e7eb]" : ""
                      }`}
                    >
                      {c.label}
                    </th>
            ...
            <tbody className="divide-y divide-[#e5e7eb]">
              {rows.map((r) => (
                <tr key={r.owner_id} className="text-[#374151]">
                  <td className="py-2 pr-3 font-medium text-[#111827]">
                    {r.vendedor}
                  </td>
                  {grupos.map((g, gi) =>
                    g.cols.map((c, ci) => (
                      ...
                      >
                        <div className="flex items-center justify-end gap-1 rounded-md border border-[#e5e7eb] bg-[#ffffff] px-2 py-1 focus-within:border-[#2dd4bf]">
                          {c.prefix && (
                          ...
                          <input
                            ...
                            className="w-14 bg-transparent text-right text-sm tabular-nums text-[#111827] outline-none"
                          />
                          {c.suffix && (
                            <span className="text-[11px] text-[#6b7280]">
                              {c.suffix}
                  ...
                  <td className="py-1.5 pl-2 text-right">
                    <button
                      ...
                      className="rounded p-1 text-[#6b7280] transition-colors hover:bg-[#e50914]/15 hover:text-[#e50914]"
          ...
          <div className="mt-4 flex items-center gap-2">
            <select
              ...
              className="rounded-lg border border-[#e5e7eb] bg-[#ffffff] px-3 py-2 text-sm text-[#111827] outline-none focus:border-[#2dd4bf] disabled:opacity-40"
            >
              <option value="">
            ...
            <button
              ...
              disabled={!addId}
              className="rounded-lg border border-[#e5e7eb] px-3 py-2 text-sm font-medium text-[#374151] transition-colors hover:bg-[#e5e7eb] disabled:cursor-not-allowed disabled:opacity-40"
            >
              + Adicionar
            </button>
          </div>
        ...
        <div className="flex items-center justify-end gap-2 border-t border-[#e5e7eb] px-6 py-4">
          <button
            onClick={onClose}
            disabled={salvando}
            className="rounded-lg px-4 py-2 text-sm font-medium text-[#374151] transition-colors hover:bg-[#e5e7eb] disabled:opacity-40"
          >
            Fechar
          </button>
          <button
